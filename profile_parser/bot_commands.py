...
def register_commands(
    ...
    def _fl_page(chat_id: int) -> tuple[str, list]:
        ...
        try:
            fl = (fl_bot_ref or [None])[0]
            if fl is None:
                raise RuntimeError("Фриланс-бот не запущен")
            sources = fl.storage.get_enabled_sources()
            kw = fl.storage.get_active_keywords()
            sw = fl.storage.get_stop_words()
            stats = fl.storage.stats()
            subs = fl.storage.subscribers()
    ...
    def _fl_sources_page() -> tuple[str, list]:
        try:
            ...
            if fl is None:
                raise RuntimeError("Фриланс-бот не запущен")
            all_sources = fl.storage.get_sources()
            lines = "\n".join(
    ...
    def _fl_storage():
        """Возвращает живой storage freelancer_bot — инвалидирует кэш автоматически."""
        fl = (fl_bot_ref or [None])[0]
        if fl is None:
            raise RuntimeError("Freelancer bot не запущен")
        return fl.storage
    ...
    async def cmd_fl_add(event: events.NewMessage.Event) -> None:
        ...
        try:
            ...
            _fl_storage().add_source(handle, title)
    ...
    async def cmd_fl_remove(event: events.NewMessage.Event) -> None:
        ...
        try:
            handle = event.pattern_match.group(1).lstrip("@")
            _fl_storage().remove_source(handle)
    ...
    async def cmd_fl_add_kw(event: events.NewMessage.Event) -> None:
        ...
        try:
            ...
            _fl_storage().add_keyword(word, weight=2, is_stop=False)
    ...
    async def cmd_fl_remove_kw(event: events.NewMessage.Event) -> None:
        ...
        try:
            word = event.pattern_match.group(1).strip().lower()
            _fl_storage().remove_keyword(word)
    ...
    async def callback_handler(event: events.CallbackQuery.Event) -> None:
        ...
        # Фриланс
            ...
            try:
                fl = (fl_bot_ref or [None])[0]
                if fl is None:
                    await event.answer("❌ Фриланс-бот не запущен")
                    return
                subs = fl.storage.subscribers()
                if event.chat_id in subs:
                    fl.storage.remove_subscriber(event.chat_id)
                    await event.answer("⏸ Фриланс на паузе")
                else:
                    fl.storage.add_subscriber(event.chat_id)
                    await event.answer("▶️ Фриланс запущен — шлю последние лиды...")
                    asyncio.create_task(fl.trigger_catchup(event.chat_id))
            except Exception as e:
