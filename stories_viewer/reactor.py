...
class CommentReactor:
    ...
    async def run_once(self, chats: list[str], limit: int = LIMIT_PER_RUN) -> int:
        ...
        for chat_handle in chats:
            ...
            # Трекаем по post_id чтобы не возвращаться к посту никогда
            to_react = []
            for post_id, post_comments in posts.items():
                # chat_str берём из первого коммента этого поста
                sample_msg = post_comments[0]
                chat_str = str(getattr(sample_msg.peer_id, 'channel_id', sample_msg.chat_id))
                if self.storage.is_reacted(chat_str, post_id, self.account_label):
                    continue
                to_react.append((post_id, chat_str, random.choice(post_comments)))
            ...
            log.info("[%s] Чат %s — %d постов, ставим реакции на %d",
                     self.account_label, chat_handle, len(posts), len(to_react))

            for post_id, chat_str, msg in to_react:
                success = await self._react(entity, msg)
                if success:
                    # Помечаем ПОСТ целиком — _react() уже залогировал коммент,
                    # здесь логируем post_id чтобы не возвращаться к посту
                    self.storage.log_reaction(chat_str, post_id, self.account_label)
                    total += 1
